...
author: Matthew Salganik and Robin Lee ^[based on the activities from SICSS 2017 - 2019 with input from Matthew Salganik, Yo-Yo Chen, Janet Xu, and Cambria Naslund).]
...
cell_based_long <- cell_based %>% pivot_longer(COVIDNEWSSWITCH.1:ALG_JOBCANDIDATE.1, names_to = "qid", 
               values_to = "mean")
...
sample_counts <- data %>% select(-educ) %>%
  group_by(sex, region, age_cat, race) %>%
  summarise_all(list(~mean(.,na.rm = T)))
...
# create a character vector of the 22 question names
# these question names can be found in the column names of the data
relevant_questions <- colnames(data)[!colnames(data) %in% c("sex", "age_cat", "region", "race","educ")]
# create container
...
# loop through
# TODO: when renaming the outcome column, different versions of R seem to have inconsistency
for (i in relevant_questions) {
  # get outcome
  outcome <- data_factor[ , i] %>% as.vector()
  colnames(outcome) <- "outcome"
  # fit model
  temp_df <- data_factor %>% select(sex, age_cat, region, race) %>%
    cbind(outcome)
...
for (i in relevant_questions) {
  ...
  colnames(outcome) <- "outcome"
  temp_df <- data %>% select(sex, age_cat, race, region) %>%
    cbind(outcome)
